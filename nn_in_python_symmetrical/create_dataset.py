...
MIN_VAL = -7
MAX_VAL = 7
# Exclude 0 from allowed values
ALLOWED_VALUES = [v for v in range(MIN_VAL, MAX_VAL + 1) if v != 0]

def is_valid(value):
    """Check if value is within the allowed range (excluding 0)."""
...
def generate_normal_sequence(seq_len=10, max_step=4):
    ...
    All values stay within [MIN_VAL, MAX_VAL] and are never 0.
    """
    seq = []
    # Start from a random allowed value (no 0)
    current = np.random.choice(ALLOWED_VALUES)
    seq.append(current)
    
    for _ in range(seq_len - 1):
        # Possible next values: within max_step, within bounds, and not 0
...
def generate_anomalous_sequence(seq_len=10, max_step=4, 
                                ...
                                num_anomalies=(2, 4)):
    """
    Start with a normal sequence, then replace `k` positions (2 to 4)
    ...
    All values stay within [MIN_VAL, MAX_VAL] and are never 0.
    """
    # Generate a normal base sequence (already contains no 0)
    ...
    for pos in positions:
        ...
        # new is within allowed range, and new != 0.
        candidates = [v for v in ALLOWED_VALUES 
