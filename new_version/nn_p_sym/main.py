...
# Activation1 scale (after ReLU, range [0, max])
...
# ------------------------------------------------------------
def quantize_weight_symmetric_per_channel(w):
    out_c, in_c = w.shape
    w_q = torch.empty_like(w, dtype=torch.int8)
    scales = torch.empty(out_c)
    for i in range(out_c):
        scale = symmetric_scale(w[i])
        q = torch.round(w[i] / scale).clamp(QMIN, QMAX)
        w_q[i] = q.to(torch.int8)
        scales[i] = scale
    return w_q, scales

W1_q, S_w1 = quantize_weight_symmetric_per_channel(model.fc1.weight.data)
W2_q, S_w2 = quantize_weight_symmetric_per_channel(model.fc2.weight.data)

# Bias quantization (scale = S_w * S_prev)
...
def compute_multiplier_shift(S_prev, S_w, S_next):
    """Given scales, compute M = S_prev * S_w / S_next.
       Return (M0_int, n) where M0_int is 31-bit fixed-point representation."""
    M = S_prev * S_w / S_next
    # Find n such that 0.5 <= M * 2^n < 1
    n = 0
    while M < 0.5:
        M *= 2
        n += 1
...
# Layer 1 multiplier (input -> activation1) - PER CHANNEL
M0_1_list = []
n_1_list = []
...
# Layer 2 multiplier (activation1 -> output) - PER CHANNEL
# For output, we need an output scale. Let's compute from the original output range
# Get output range from model
with torch.no_grad():
    all_outputs = []
    for x, _ in train_loader:
        out = model(x)
    ...
    S_out = all_outputs.max().item() / 7.0  # scale for output (0-1 range, but symmetric? Actually output is 0-1 from sigmoid)
    # For sigmoid output, we can use scale = 1/256 for fixed-point

# Use a reasonable output scale (Q0.8 format: 8 fractional bits)
S_out = 1.0 / 256.0
...
# ------------------------------------------------------------
# Quantized forward pass (using integer arithmetic simulation)
# ------------------------------------------------------------
def quantized_forward_int_sim(x_float):
    ...
    # x_q shape: [batch, 10], W1_q shape: [5, 10], we want [batch, 5]
    ...
    for i in range(acc1.shape[1]):  # iterate over 5 channels
    ...
    # Dequantize to float for sigmoid (in real deployment you'd use fixed-point sigmoid)
