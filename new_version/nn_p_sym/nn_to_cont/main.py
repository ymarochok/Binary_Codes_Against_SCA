...
def project_weights(model):
    with torch.no_grad():
        for name, param in model.named_parameters():
            if 'weight' in name:   # only project weights, not biases
                # Find index of nearest allowed value
                diff = param.unsqueeze(-1) - ALLOWED_VALS.to(param.device)
                idx = torch.abs(diff).argmin(dim=-1)
                param.copy_(ALLOWED_VALS[idx])
...
def quantize_weight_symmetric_per_channel_no_zero(w):
    ...
    for i in range(out_c):
        # Symmetric scale: max_abs / 7
        scale = w[i].abs().max().item() / 7.0
        # Normalised weights (floating point, roughly in [-7, 7])
        w_norm = w[i] / scale
        # Find nearest allowed value for each weight element
        diff = w_norm.unsqueeze(-1) - allowed   # shape [in_c, 14]
        idx = torch.abs(diff).argmin(dim=-1)    # shape [in_c]
